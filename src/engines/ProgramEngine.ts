...
import { RetrieveMemory, SaveMemory, SearchMemory } from '../tools/memory';
...
const searchMemoryTool = new SearchMemory(retrievalApiUrl, retrievalApiKey);
...
export class ProgramEngine {
    ...
    private memoryMap : Record<string, Tool> // a map of memory tools that the agent can use
    private pastPages: string[] = []; // a list of pages that have been loaded from memory
    private currentMemory: string = ''; // the current memory page that the agent is working on
    ...
    Memory Actions:
    - You can access memory pages by their ID using RetrieveMemory[page_id, page_id, ...]
    - When necessary, load a memory page from external storage and the memory tool will return the content of the page
    - You can search for memory pages via semantic search using SearchMemory[query]
    - You can save a memory page to external storage using SaveMemory[content], you will retrieve the ID of the page in return.
    - Fetched memory pages will be available for the next prompt, and will be removed after the prompt is completed.
    - You will have a list of memory pages that you have accessed and can retrieve once again should you need them.
    Past Pages: 
    {{past_pages}}
    Current Memory: 
    {{memory}}
    ...
    Rules:
    - If you have received an Input from the user, you should reply with a Thought, an Action, and optionally a Memory Action.
    - If you have received an Observation from a tool, you should reply with a Thought, an Action, and optionally a Memory Action.
...
Action: Search[weather today]`,
    ...
    private async plan(input: string): Promise<[string, string, string?, string?]> {
        logger.debug(`Planning for input: ${input}`);
        // copy system prompt and replace variables
        const pastPages = this.pastPages.map((o) => `- ${o}`).join("\n");
        const currentSystemPrompt = this.systemPrompt.replace("{{past_pages}}", pastPages).replace("{{memory}}", this.currentMemory);
        ...
        const [memoryAction, memoryActionInput] = this.parseMemoryAndInput(plan);
        if (memoryAction && memoryActionInput) {
            console.log(`PLAN -- Parsed Action: ${action}, Parsed Action Input: ${actionInput}`)
            console.log(`PLAN -- Parsed Memory Action: ${memoryAction}, Parsed Memory Action Input: ${memoryActionInput}`)
            return [action, actionInput, memoryAction, memoryActionInput];
        }
    ...
    private async memory(action: string, memoryInput: string) {
        const tool = this.memoryMap[action];
        if (!tool) {
            throw new Error(`Could not find tool: ${action}`);
        }
        logger.debug(`MEMORY -- Acting with tool: ${tool.name} and input: ${memoryInput}`)
        const observation = await tool.fn(memoryInput);
        logger.debug(`MEMORY -- Observation: ${observation}`);
        // save the observation to the agent's memory
        this.InternalDialogue.addInteraction(``, observation);
        return observation;
    }
    ...
    private async react(input: string): Promise<string> {
        ...
        let [action, actionInput, memoryAction, memoryActionInput] = await this.plan(`Input: ${input}`);
        logger.debug(`React -- Planned Action: ${action}, Action Input: ${actionInput}`)
        if (action === "Finish") {
            return actionInput;
        }
        if (memoryAction && memoryActionInput) {
            const memoryObservation = await this.memory(memoryAction, memoryActionInput);
            if (memoryAction === "RetrieveMemory") {
                this.pastPages.push(memoryActionInput);
                this.currentMemory = memoryObservation;
            }
            logger.debug(`React -- Memory Observation: ${memoryObservation}`);
        }
        for (let i = 0; i < this.maxIterations; i++) {
            ...
            [action, actionInput, memoryAction, memoryActionInput] = await this.plan(`Observation: ${observation}`);
            if (memoryAction && memoryActionInput) {
                const memoryObservation = await this.memory(memoryAction, memoryActionInput);
                if (memoryAction === "RetrieveMemory") {
                    if (memoryActionInput !in this.pastPages) {
                        this.pastPages.push(memoryActionInput);
                    }
                    this.currentMemory = memoryObservation;
                }
                if (memoryAction === "SaveMemory") {
                    this.currentMemory = "";
                    if (memoryActionInput !in this.pastPages) {
                        this.pastPages.push(memoryActionInput);
                    }
                }
                if (memoryAction === "SearchMemory") {
                    this.currentMemory = memoryObservation;
                }
                logger.debug(`React -- Memory Observation: ${memoryObservation}`);
            }
    ...
    parseMemoryAndInput(text: string): [string, string] {
        const regex = `Memory: (${Object.keys(this.memoryMap)
            .reduce((acc, a, i) => {
                if (i === 0) {
                    return acc + a;
                }
